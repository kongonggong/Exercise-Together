...
private struct DualVideoSection: View {
        ...
        .onChange(of: userVideoSelection) { _, newValue in
            Task {
                if let videoFile = try? await newValue?.loadTransferable(type: VideoTransferable.self) {
                    let uploadedPlayer = AVPlayer(url: videoFile.url)
                    await MainActor.run {
                        userVideoURL = videoFile.url
                        userPlayer = uploadedPlayer
                        resetPlayersForSynchronizedPlayback()
                    }
                }
            }
        }
    ...
    private func resetPlayersForSynchronizedPlayback() {
        isPlaying = false
        expertPlayer?.pause()
        userPlayer?.pause()
        expertPlayer?.seek(to: .zero, toleranceBefore: .zero, toleranceAfter: .zero)
        userPlayer?.seek(to: .zero, toleranceBefore: .zero, toleranceAfter: .zero)
    }
}
...
private struct SharedPlaybackControls: View {
        ...
        HStack(spacing: 28) {
            ...
            Button {
                withAnimation(.spring(response: 0.3, dampingFraction: 0.6)) {
                    isPlaying.toggle()
                    if isPlaying {
                        seekToStartIfFinished(expertPlayer)
                        seekToStartIfFinished(userPlayer)
                        expertPlayer?.play()
                        userPlayer?.play()
                    } else {
                        expertPlayer?.pause()
    ...
    private func seekToStartIfFinished(_ player: AVPlayer?) {
        guard let player,
              let duration = player.currentItem?.duration.seconds,
              duration.isFinite,
              player.currentTime().seconds >= duration - 0.05 else {
            return
        }
